...
export default function Portfolio() {
  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-12 text-[color:var(--foreground)]">
      <div className="flex flex-col md:flex-row gap-4 md:items-end md:justify-between mb-8">
        <div>
          <h1 className="text-3xl font-semibold text-primary">Our Work</h1>
          <p className="text-sm mt-1 text-[color:var(--muted-foreground)]">Browse recent projects. Use search and filters to refine results.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <Link key={project.id} href={`/project/${project.id}`} className="group block border border-[color:var(--border)] bg-[color:var(--card)] rounded-lg overflow-hidden hover:shadow-md hover:border-transparent hover:outline hover:outline-2 hover:outline-[color:var(--ring)] transition">
                ...
                <div className="p-4">
                  <div className="flex items-center justify-between">
                    ...
                    {project.status && (
                      <span className={`text-xs px-2 py-1 rounded capitalize ${getStatusChipClass(project.status)}`}>{project.status.replace('_',' ')}</span>
                    )}
                  </div>
                  {project.description ? (
                    <p className="text-sm text-[color:var(--muted-foreground)] line-clamp-2">{project.description}</p>
                  ) : null}
                </div>
              </Link>
            );
          ...
          {filtered.length === 0 && (
            <div className="col-span-full text-center py-16 border border-dashed border-[color:var(--border)] rounded-lg">
              ...
              <p className="text-sm text-[color:var(--muted-foreground)] mt-1">Try adjusting your search or filters.</p>
            </div>
          )}
        </div>
      )}
